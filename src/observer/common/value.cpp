...
void Value::set_data(char *data, int length)
{
  switch (attr_type_) {
    ...
    case AttrType::DATES: { //接受char*或int* 转换为 统一的char* 格式，仅在set_data()传入的参数是int*或data取出的参数是int*时，对point_value进行
      if(length == 4)
      {
        int year,month,day;
        int date = *reinterpret_cast<const int*>(data);
        year = (date >> 16) & 0xffff;
        month = (date >> 8) & 0xff;
        day = date & 0xff;
        ...
        set_date(result.c_str(), result.length());
      }
      else
      {
...
{
  reset();
  if (!common::is_valid_date_format(s))
  {
    attr_type_ = AttrType::UNDEFINED;
    return;
  }
...
void Value::set_value(const Value &value)
{
  switch (value.attr_type_) {
    ...
    case AttrType::DATES: {
      set_date(value.data(),value.length_);
    }
    default: {
...
const char *Value::data() const
{
  switch (attr_type_) {
    ...
    case AttrType::DATES:{
      const char* int_val = common::translate_date(value_.pointer_value_);
      return int_val;
    }
    default: {
