...
type:
    ...
    | DATE_T   { $$ = static_cast<int>(AttrType::DATES); } //DATE parse
    ;
...
value:
    ...
    |DATE { //对lex过来的DATE进行更加细致的检查
      char *tmp = common::substr($1,1,strlen($1)-2);
      if(common::is_valid_date_format(tmp))
      {
        // char* data = common::translate_date(tmp);
        $$ = new Value(AttrType::DATES, tmp , strlen(tmp));
        // delete[] data;
      }
      else
      $$ = new Value(AttrType::UNDEFINED, tmp);
      free(tmp);
    }
    |SSS {
      char *tmp = common::substr($1,1,strlen($1)-2);
