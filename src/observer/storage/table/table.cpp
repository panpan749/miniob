...
RC Table::drop(const char *name)
{
  ...
  RC rc = RC::SUCCESS;
  string file_data = table_data_file(base_dir, name);
  if (rc != RC::SUCCESS) {
    LOG_WARN("Failed to close file %s", file_data.c_str());
    return rc;
  }
  ::remove(file_data.c_str());
  ...
  for(auto index : indexes_)
    index->destroy();
  string meta_file = table_meta_file(base_dir, name);
